...
(function(){
  ...
  function wireOverview(){
    ...
    const btn=document.querySelector('[data-v2-content]'); if(btn) btn.addEventListener('click',()=>{location.hash='content'});
  }
  ...
  function enhanceRecommendations(){
    ...
    root.querySelectorAll('.recommendation').forEach((card,i)=>{
      ...
      const old=card.querySelector('.action-box');
      if(old) old.classList.add('v2-action-box');
      if(!card.querySelector('.impact-strip')){
        const strip=document.createElement('div'); strip.className='impact-strip';
        strip.innerHTML=`<div><small>Impacto esperado</small><b>${i===0?'Reducir desperdicio de presupuesto':i===1?'Recuperar demanda existente':'Aumentar rendimiento de contenido'}</b></div><div><small>Horizonte</small><b>${i===0?'7 días':'14 días'}</b></div><div><small>Confianza</small><b>${i===0?'Alta':'Media-alta'}</b></div>`;
        card.prepend(strip);
      }
    });
  }
  ...
  function patchGlobal(){
    ...
    const original=window.renderRoute;
    if(!original || window.__mhV2Wrapped) return;
